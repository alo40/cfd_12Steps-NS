...
Today's 05.07.2020, need to find a way to have only one file for all my functions ...
"""
...
# ------------------------------------------------------
def timeStep_NavierStokes(cx, cy, dx, dy, Co, Nu, rho):
...
# ------------------------------------------------------
def boundaries_ChannelFlow(u, v, p):

    # periodic boundaries @ x = 0
    u[ :,  0] = u[ :, -1] 
    v[ :,  0] = v[ :, -1]
    p[ :,  0] = p[ :, -1]
    
    # periodic boundaries @ x = Lx
    u[ :,  0] = u[ :, -1] 
    v[ :,  0] = v[ :, -1]
    p[ :,  0] = p[ :, -1]
    ...
    # flux boundaries (dp/dy = 0)
    
    
    return u, v, p # simple as that!
...
# ------------------------------------------------------
def stencil_NavierStokes(u, v, p, b, dx, dy, dt, Nu, rho):
